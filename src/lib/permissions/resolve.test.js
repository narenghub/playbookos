...
test('T11 — super_admin holds all 240 non-cron features (and no crons)', async () => {
  ...
  assert.equal(allowedNonCron, 240); // +5 Content Studio (Part 3) +1 golfnex.content.publish_post (mcp_tool) +6 Prospects (page+list/get/update/run/qualify) +3 Notifications (list/read/read-all) +4 CPHI Event Agent (page+exhibitors.list/thin-supply.list/exhibitors.update) +1 personal.auth_password.update (self-service, every role) +2 AROS establishments (page+list)
