...
async function gatherBriefingData() {
  ...
  const newOrders = (await query(
    `SELECT id, order_date, amount, buyer_type, product_category, notes, created_at
     FROM orders WHERE created_at::timestamptz >= NOW() - INTERVAL '24 hours' ORDER BY created_at DESC`
  )).rows;
  ...
  const newSkus = (await query(
    `SELECT id, name, category, supplier, sale_price FROM skus WHERE created_at::timestamptz >= NOW() - INTERVAL '24 hours' ORDER BY created_at DESC`
  )).rows;
