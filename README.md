...
**Market intelligence** — the weekly 150-molecule engine (`runMarketIntelligence`, its own Monday 09:00 CST / 15:00 UTC cron — `weekly-mon-15utc-market-intelligence`). Each run asks Claude (`claude-opus-4-7`, batched) for **100 research chemicals (non-GMP) + 50 GMP generic APIs** across all therapeutic areas, deduped against `molecule_history` (best-effort, name+CAS) using a rolling 12-week exclusion window so molecules don't repeat, cross-checked against the catalog, stored in `molecule_history`, with the top-20 research + top-10 GMP queued as `approval_queue` sourcing tasks for Palash and a summary emailed to the CEO. **CAS numbers are AI-generated and must be verified before sourcing.** Endpoints: `POST /market/analyze` (procurement; async, returns 202 — runs ~2-5 min), `GET /market/weekly` (procurement **or** intelligence — current week's two tabs), `GET /market/history` (procurement; filters: week/gmp_status/sourcing_status/category), `PUT /market/molecule/:id` (procurement; update sourcing status + supplier), `GET /market/gaps` (procurement or intelligence — not-in-catalog, demand-ranked), `GET /market/export/research` and `GET /market/export/gmp` (procurement; CSV)
...
`runEmailEngine({weekStart, dryRun, topMolecules})` turns weekly demand signals into per-segment A/B email campaigns. Top **10** molecules x 4 segments = **40 campaigns / 80 variants** per week.

**Demand signals (three sources).** Top 20 GSC queries by impressions over the trailing 30 days (`seo_rankings`), the top 40 molecules from this week's `molecule_history`, and the full Algolia `abiozen_products` marketplace index (~114 records). Deduplicated by normalised name; priority is encoded in score bands so GSC-backed molecules (buyers already searching) outrank the market-intelligence feed, which outranks catalog-only molecules. A molecule in several sources accumulates score and collects every source tag — surfaced as **GSC Signal / Market Intel / In Catalog** badges on each campaign card. GSC rows are search *queries*, not molecules, so each query is resolved against a vocabulary built from the active catalog and the trailing 12 weeks of `molecule_history`. Matching is boundary-aware rather than plain substring: chemical names embed each other (`2-(1-aminocyclobutyl)-5-bromopyrimidine` contains `5-bromopyrimidine` but is a different compound), so hyphens count as token characters while other punctuation collapses — `4'-fluoroacetophenone` still matches `4-fluoroacetophenone` across the apostrophe. Queries resolving to nothing are counted (`seo_matched`) and dropped, never guessed at. On the current corpus 8 of 20 queries resolve.
...
**Generation.** One Claude call (`claude-opus-4-8`) per molecule × segment returns *both* variants as JSON — **copy only** (subjects + body paragraphs), not HTML. A deterministic `generateEmailHtml()` assembles the branded template: navy header + base64 Abiozen logo, molecule hero (name / CAS / purity + GMP + COA badges), a 5-row spec table, the copy, a green "Request Quote" mailto CTA, a "View Full Catalog" button, and a CAN-SPAM footer with unsubscribe. This keeps all 80 variants pixel-identical, guarantees the CTA/footer are never missing, and keeps every availability claim catalog-derived (unknown specs render "On request", never fabricated). Copy is sanitised (`<script>`/`<iframe>`/`on*`/`javascript:` stripped) before templating.

**Logo deliverability:** the logo is base64-embedded per the original spec, which renders in the in-app preview and Apple Mail but is **stripped by Gmail and Outlook**. `LOGO_SRC` in `email-assets.js` is a one-line switch to the hosted `public/abiozen-logo.png` (served at `playbook.abiozen.com/abiozen-logo.png`) for universal rendering.
...
# Thu Jul 23 18:46:57 CDT 2026
# Thu Jul 23 18:49:28 CDT 2026
